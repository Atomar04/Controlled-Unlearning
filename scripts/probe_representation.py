...
@torch.no_grad()
def extract_features(agent: PPOUnlearner, states: np.ndarray, batch_size: int = 4096) -> np.ndarray:
    feats = []
...
def collect_dataset(
    ...
    # Important: shuffle before saving so class order / late-episode skew cannot
    # accidentally leak into any downstream split or inspection.
...
def make_splits(
    ...
    for split_name, idx in best.items():
        ...
        if cnts.min() < 25:
            raise RuntimeError(f"{split_name} split has too few examples of one class: {dict(zip(vals, cnts))}")
...
def cmd_fit(args):
    ...
    env = make_env(args.env, n_envs=1)
    agent = load_agent(env, args.model_path)
    feats = extract_features(agent, states, batch_size=args.batch_size)
...
def build_parser():
    ...
    f.add_argument("--dataset", type=str, required=True)
    f.add_argument("--model_path", type=str, required=True)
    f.add_argument("--out", type=str, required=True)
    f.add_argument("--metrics_out", type=str, default=None)
